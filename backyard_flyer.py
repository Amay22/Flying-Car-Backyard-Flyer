...
class BackyardFlyer(Drone):
    ...
    def local_position_callback(self):
        ...
        """
        if self.flight_state == States.TAKEOFF and \
           self.local_position[2] * -1.0 > self.target_position[2] * 0.95:
            if len(self.all_waypoints) == 0:
                self.all_waypoints = self.calculate_box()
            self.waypoint_transition()
        elif self.flight_state == States.WAYPOINT and \
             np.linalg.norm(self.target_position[0:2] - self.local_position[0:2]) < 0.2:
            if len(self.all_waypoints) > 0:
                self.waypoint_transition()
